...
@pytest.mark.unit
def test_cli_vm_power_dry_run_makes_no_call(monkeypatch):
    from nutanix_aiops.cli import app

    result = runner.invoke(app, ["vm", "power", "v1", "on", "--dry-run"])
    assert result.exit_code == 0, result.output
    assert "DRY-RUN" in result.output
    assert "power-on".split("-")[0] in result.output.lower()


@pytest.mark.unit
def test_cli_vm_migrate_dry_run_shows_target(monkeypatch):
    from nutanix_aiops.cli import app

    result = runner.invoke(app, ["vm", "migrate", "v1", "h-new", "--dry-run"])
    assert result.exit_code == 0, result.output
    assert "DRY-RUN" in result.output
