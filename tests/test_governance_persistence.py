...
throwaway home (``NUTANIX_AIOPS_HOME``) and assert that the rows compliance
...
from nutanix_aiops.governance import (
    PolicyDenied,
    capture_prior_state,
    governed_tool,
    mark_unknown,
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NUTANIX_POLICY_DISABLED", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="pc-prod")
...
def test_high_risk_denied_without_approver_and_denial_is_audited(gov_home):
    ...
    with pytest.raises(PolicyDenied, match="requires 'dual' approval"):
        _drop_widget(name="w2", target="pc-prod")
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("NUTANIX_AUDIT_APPROVED_BY", "ops-alice")
    result = _drop_widget(name="w3", target="pc-prod")
...
def test_operator_rules_file_restores_tier_none_for_high_risk(gov_home):
    ...
    _reset_singletons()
    result = _drop_widget(name="w4", target="pc-prod")
    assert result["status"] == "dropped"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: vm_update must capture the
    VM's prior CPU/memory before resizing and persist the restore-undo on disk."""
    monkeypatch.setenv("NUTANIX_AUDIT_APPROVED_BY", "ops-alice")
    conn = MagicMock(name="conn")
    conn.get_with_etag.return_value = (
        {"data": {"extId": "v1", "name": "web01", "numSockets": 2,
    ...
    result = gov.vm_update(vm_ext_id="v1", num_sockets=4)
    assert result["priorState"] == {"numSockets": 2, "memoryBytes": 4294967296}
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "vm_update"
    undo_params = json.loads(undo[0]["undo_params"])
    assert undo_params["num_sockets"] == 2
    assert undo_params["memory_bytes"] == 4294967296

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["vm_update"]
...
@pytest.mark.unit
def test_self_lockout_refusal_writes_no_undo_and_audits_as_error(gov_home, monkeypatch):
    """The Prism Central self-lockout guard, end-to-end through the governed tool.

    This is the path the CLI takes too (``cli/vm.py`` delegates to these same
    governed functions), so proving it here proves BOTH entry points. The refusal
    must reach the caller as an error payload, must POST nothing to Prism
    Central, and must leave NO undo token behind — recording a vm_power_on
    inverse for a power-off that never happened would be a lie, and it is exactly
    the unapplicable token this guard exists to prevent.
    """
    from mcp_server.tools import vms as gov
    ...
    result = gov.vm_power_off(vm_ext_id="vm-pc")
    assert "Refusing to power off" in result["error"]
    assert "hypervisor console" in result["error"]  # remedy survived the 300-char cap
    conn.post.assert_not_called()
    assert result.get("_undo_id") is None

    assert not (gov_home / "undo.db").exists() or not _rows(gov_home / "undo.db", "undo_log")
    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["vm_power_off"]
