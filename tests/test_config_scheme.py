"""a Prism Central behind a TLS-terminating proxy is plain HTTP; don't hardcode it.
...
def test_scheme_defaults_to_https_so_existing_configs_are_unchanged():
    t = TargetConfig(name="n1", host="h", port=9440)
...
def test_scheme_http_is_honoured():
    t = TargetConfig(name="n1", host="h", port=8080, scheme="http")
...
def test_invalid_scheme_is_rejected_at_construction():
    with pytest.raises(ValueError, match="scheme must be"):
        TargetConfig(name="n1", host="h", port=9440, scheme="ftp")
