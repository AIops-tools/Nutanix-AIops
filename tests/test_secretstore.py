...
nothing touches the real ``~/.nutanix-aiops``.
"""
...
def test_migrate_legacy_env(store_dir):
    (store_dir / ".env").write_text(
        "NUTANIX_NAS1_APIKEY=legacy-pass\n# comment\nFOO=bar\n"
    )
    imported = ss.migrate_legacy_env("NUTANIX_", "_APIKEY", "pw")
    assert imported == ["nas1"]
    assert ss.SecretStore.unlock("pw").get("nas1") == "legacy-pass"
