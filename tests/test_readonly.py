...
from nutanix_aiops.governance import READ_ONLY_ENV, is_read_only
from nutanix_aiops.governance.decorators import PolicyDenied, governed_tool
...
def test_governed_write_is_refused_in_read_only(read_only):
    """A write is refused before it can reach Prism Central — from any caller."""
    calls = []

    @governed_tool(risk_level="high")
    def delete_everything(target: str = "pc-lab") -> dict:
        calls.append(target)
        return {"deleted": True}
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def list_things(target: str = "pc-lab") -> dict:
        return {"vms": [{"name": "thing"}], "returned": 1}

    assert list_things()["returned"] == 1


@pytest.mark.unit
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. undo_apply, vm_power_on) is refused too."""

    @governed_tool(risk_level="medium")
    def restart_thing(target: str = "pc-lab") -> dict:
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        # Destructive Prism Central lifecycle writes must be gone …
        for name in ("vm_delete", "vm_power_on", "snapshot_delete", "subnet_delete",
                     "storage_container_delete", "lcm_update", "pd_failover"):
            assert name in dropped, f"{name} is a write and must be unregistered"
        # … while the inventory reads an agent needs stay.
        for name in ("vm_list", "vm_get", "cluster_list", "host_list",
                     "cluster_health_rca", "alert_triage_rca"):
            assert name not in dropped, f"{name} is a read and must stay exposed"
...
def test_risk_level_agrees_with_read_write_docstring_tag():
    ...
    read-only mode would expose something the docs call a write — which is
    exactly what happened here before this test existed (alert_acknowledge,
    alert_resolve, category_create, snapshot_create, lcm_precheck and
    vm_power_on were all tagged [WRITE] while carrying risk_level="low").
    """
    from mcp_server import server
