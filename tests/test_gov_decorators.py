...
``nutanix_aiops.governance.*`` and binds state to an isolated home via
``NUTANIX_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import nutanix_aiops.governance.budget as budget_mod
import nutanix_aiops.governance.decorators as dec_mod
...
from nutanix_aiops.governance.budget import BudgetExceeded
...
def dec_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NUTANIX_MAX_TOOL_CALLS", raising=False)
    monkeypatch.delenv("NUTANIX_MAX_TOOL_SECONDS", raising=False)
    _reset()
    yield tmp_path
    _reset()
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("NUTANIX_MAX_TOOL_CALLS", "0")
...
def test_policy_disabled_marks_status_bypassed(dec_home, monkeypatch):
    monkeypatch.setenv("NUTANIX_POLICY_DISABLED", "1")
    _reset()
