...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/nutanix-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install nutanix-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/nutanix-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [nutanix-aiops, uvx]` — either one suffices — with every variable kept
