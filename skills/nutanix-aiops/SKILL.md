...
- **Secure by default (v0.2.0+)**: with no `~/.nutanix-aiops/rules.yaml`, high/critical operations are denied unless `NUTANIX_AUDIT_APPROVED_BY` names an approver (set `NUTANIX_AUDIT_RATIONALE` too). `nutanix-aiops init` seeds a starter rules.yaml; an operator-authored rules file is honoured as-is.
