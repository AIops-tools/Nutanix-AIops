"""Encrypted secret storage for nutanix-aiops.

Secrets (Nutanix API keys) are NEVER written to disk in plaintext. They
live in ``~/.nutanix-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``NUTANIX_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "nutanix-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "NUTANIX_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``NUTANIX_<TARGET>_PASSWORD``; ``prefix='NUTANIX_'`` and
