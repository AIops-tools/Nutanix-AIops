...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``NUTANIX_RUNAWAY_MAX`` times within ``NUTANIX_RUNAWAY_WINDOW_SEC`` trips a
...
from nutanix_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing Nutanix-specific; the decorator wraps it into a
