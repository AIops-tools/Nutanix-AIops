"""The ``@governed_tool`` decorator — mandatory wrapper for all Nutanix MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.nutanix-aiops/audit.db``
...
from nutanix_aiops.governance.audit import detect_agent, get_engine
from nutanix_aiops.governance.budget import BudgetExceeded, get_budget
from nutanix_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from nutanix_aiops.governance.patterns import PatternMatch, get_pattern_engine
from nutanix_aiops.governance.policy import PolicyResult, get_policy_engine
from nutanix_aiops.governance.readonly import is_read_only, read_only_denial
from nutanix_aiops.governance.sanitize import sanitize

_log = logging.getLogger("nutanix-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all Nutanix MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.nutanix-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("NUTANIX_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("NUTANIX_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (NUTANIX_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set NUTANIX_AUDIT_APPROVED_BY to the authorizing human (and "
            f"NUTANIX_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`nutanix_aiops.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from nutanix_aiops.governance.undo import get_undo_store
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight Nutanix calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``nutanix_aiops.ops.jobs`` → ``nutanix-aiops``
    ``mcp_server.server`` → ``nutanix-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("nutanix_aiops") or module.startswith("mcp_server"):
        return "nutanix-aiops"
    return "unknown"
