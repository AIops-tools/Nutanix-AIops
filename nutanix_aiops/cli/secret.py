"""``nutanix-aiops secret`` — manage the encrypted credential store.

Secrets (Nutanix API keys) are stored in ``~/.nutanix-aiops/secrets.enc``
...
from nutanix_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: nutanix-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update NUTANIX_AIOPS_MASTER_PASSWORD.[/]")
