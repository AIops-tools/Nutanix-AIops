"""Environment and connectivity diagnostics for Nutanix AIops."""

from __future__ import annotations

from rich.console import Console

from nutanix_aiops.config import CONFIG_FILE, ENV_FILE, load_config
from nutanix_aiops.secretstore import SECRETS_FILE, check_permissions, has_store
...
def run_doctor(skip_auth: bool = False) -> int:
    ...
    if not CONFIG_FILE.exists():
        ...
        _console.print("[yellow]  Run 'nutanix-aiops init' to set up your first target.[/]")
        return 1
    ...
    if has_store():
        ...
        _console.print(
            ...
            f"'nutanix-aiops secret migrate'.[/]"
        )
    else:
        _console.print(
            "[yellow]! No secret store yet. Run 'nutanix-aiops init' to set up "
    ...
    for target in config.targets:
        try:
            ...
            # List clusters — a real v4 read that also proves the account has REST
            # API rights (WebUI access does NOT imply REST access, a common gotcha).
            rows = conn.list_all("/api/clustermgmt/v4.0/config/clusters", limit=1, max_items=1)
            n = len(rows)
            _console.print(
                f"[green]✓ Connected to '{target.name}' ({target.host}:{target.port}) "
                f"— Prism Central v4 REST reachable, {n}+ cluster(s) visible[/]"
            )
